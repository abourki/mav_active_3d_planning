...
RosPlanner::RosPlanner(const ::ros::NodeHandle& nh,
    ...
    : OnlinePlanner(factory, param_map), nh_(nh), nh_private_(nh_private), _traj_generation(nh,nh_private){
    // params
    ...
    //  target_pub_ = nh_.advertise<quadrotor_msgs::Trajectory>(
    ...
    nh_private.param("planner/save_log_folder", _log_path, _log_path);
...
bool RosPlanner::requestNextTrajectory() {
    ...
    if (!OnlinePlanner::requestNextTrajectory()) {
        ...
        _logger.addTime(_logger.log_ts.elapsed_ms());
    }
...
void RosPlanner::odomCallback(const nav_msgs::Odometry& msg) {
    ...
    Eigen::Vector3d vel;
    double speed = Eigen::Vector3d(msg.twist.twist.linear.x,
                                   msg.twist.twist.linear.y,
                                   msg.twist.twist.linear.z).norm();
    _logger.log_dist_speed(speed,current_position_);
...
void RosPlanner::requestMovement(const EigenTrajectoryPointVector& trajectory) {
    ...
    quadrotor_msgs::Trajectory msg;

    double yaw= yawFromQuaternion(current_orientation_);

    Eigen::Vector4d start; start << trajectory[0].position_W, yaw;
    Eigen::Vector4d goal; goal << trajectory.back().position_W,trajectory.back().getYaw();



    mav_trajectory_generation::Trajectory traj;
    _traj_generation.computeTrajectoryAndYaw(start,Eigen::Vector4d::Zero(),goal,Eigen::Vector4d::Zero(),std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> >()
                                             ,&traj);


    _traj_generation.setTrajectory(traj);
    _traj_generation.startTrajectory();
